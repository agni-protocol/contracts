...
contract MasterChef is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @notice poolPid[token0][token1][fee] => pid
    mapping(address => mapping(address => mapping(uint24 => uint256))) poolPid;
    /// @notice poolAddressPid[poolAddress] => pid
    mapping(address => uint256) public poolAddressPid;

    /// @notice Address of AGNI contract.
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, IAgniPool indexed pool, ILMPool indexed lmPool);
    ...
    /// @param _AGNI The AGNI token contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(IERC20 _AGNI, INonfungiblePositionManager _nonfungiblePositionManager, address _WMNT) {
        AGNI = _AGNI;
    ...
    function pendingAgni(uint128 _tokenId) external view returns (uint256 reward) {
        UserPositionInfo memory positionInfo = userPositionInfos[_tokenId];
        if (positionInfo.pid != 0) {
            PoolInfo memory pool = poolInfo[positionInfo.pid];
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        if (AGNI.allowance(_receiver, address(this)) != type(uint256).max) revert();
        receiver = _receiver;
    ...
    function add(uint256 _allocPoint, IAgniPool _pool, bool _withUpdate) external onlyOwner {
        if (_withUpdate) massUpdatePools();

        ILMPool lmPool = LMPoolDeployer.deploy(_pool);

        totalAllocPoint += _allocPoint;
        address token0 = _pool.token0();
        address token1 = _pool.token1();
        uint24 fee = _pool.fee();
        if (poolPid[token0][token1][fee] != 0) revert DuplicatedPool(poolPid[token0][token1][fee]);
        ...
        poolPid[token0][token1][fee] = poolLength;
        poolAddressPid[address(_pool)] = poolLength;
        emit AddPool(poolLength, _allocPoint, _pool, lmPool);
    }

    /// @notice Update the given pool's AGNI allocation point. Can only be called by the owner.
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotAgniNFT();
        ...
        uint256 pid = poolPid[cache.token0][cache.token1][cache.fee];
    ...
    function transferToken(address _token, address _to) internal {
        ...
        if (_token == address(AGNI)) {
            unchecked {
                // In fact balance should always be greater than or equal to agniAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balance >= agniAmountBelongToMC) {
                    balance -= agniAmountBelongToMC;
                } else {
                    // This should never happend.
                    agniAmountBelongToMC = balance;
                    balance = 0;
                }
            }
        }
    ...
    function sweepToken(address token, uint256 amountMinimum, address recipient) external nonReentrant {
        ...
        if (token == address(AGNI)) {
            unchecked {
                // In fact balance should always be greater than or equal to agniAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balanceToken >= agniAmountBelongToMC) {
                    balanceToken -= agniAmountBelongToMC;
                } else {
                    // This should never happend.
                    agniAmountBelongToMC = balanceToken;
                    balanceToken = 0;
                }
            }
        }
    ...
    /// @param _amount The amount of agni injected.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        // Transfer agni token from receiver.
        AGNI.safeTransferFrom(receiver, address(this), _amount);
        // Update agniAmountBelongToMC
        unchecked {
            agniAmountBelongToMC += _amount;
        }
    ...
    /// @notice Update agni reward for all the liquidity mining pool.
    ...
    /// @notice Update agni reward for the liquidity mining pool.
    ...
    /// @notice Safe Transfer AGNI.
    /// @param _to The AGNI receiver address.
    /// @param _amount Transfer AGNI amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            ...
            // Update agniAmountBelongToMC
            unchecked {
                if (agniAmountBelongToMC >= _amount) {
                    agniAmountBelongToMC -= _amount;
                } else {
                    agniAmountBelongToMC = balance - _amount;
                }
            }
            AGNI.safeTransfer(_to, _amount);
        }
    }
