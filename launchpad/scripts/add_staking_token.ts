...
async function main() {
  ...
  // add agni
  let agniIsStakingToken = await stakingPool.isStakingToken(WMNT);
  if (!agniIsStakingToken){
      let addTx = await stakingPool.addStakingToken(WMNT);
  ...
  if (scoreAgni != WMNT) {
    let setAgniTokenTx = await scoreCalculator.setAgniToken(WMNT);
