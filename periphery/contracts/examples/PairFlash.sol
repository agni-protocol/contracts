...
import '@agniswap/core/contracts/interfaces/callback/IAgniFlashCallback.sol';
...
/// @notice An example contract using the Agni flash function
contract PairFlash is IAgniFlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `agniFlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IAgniPool pool = IAgniPool(PoolAddress.computeAddress(deployer, poolKey));
